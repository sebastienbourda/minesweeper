...
function startGame() {
  for (let i = 0; i < rowLength; i += 1) {
    ...
    for (let j = 0; j < rowLength; j += 1) {
      row += `<td id="${i}${j}" data-row="${i}" data-column="${j}" class="unopened"></td>`;
    }
    row += '</tr>';
    table.innerHTML += row;
  }
...
function initPlayground() {
  for (let i = 0; i < rowLength; i += 1) {
    playground.push([0, 0, 0, 0, 0]);
  }
}

function setMines() {
  const boxNumbers = (rowLength ** 2);
  for (let i = 0; i < boxNumbers * 0.1 + 1; i += 1) {
    const row = Math.floor(Math.random() * rowLength);
    const column = Math.floor(Math.random() * rowLength);
...
function inPlayground(x, y, i, j) {
  const inPlaygroundX = ((x + i) >= 0 && (x + i) < rowLength);
  const inPlaygroundY = ((y + j) >= 0 && (y + j) < rowLength);
...
function checkWin() {
  ...
  if (sum >= (rowLength ** 2) - ((rowLength ** 2) * 0.1)) {
    alert('you win');
  }
}
